-- mady by someone, enhanced by me
-- Gonna start leaving Update logs

-- 11/13/22 - Updated and extensified the bypass
-- 12/22/22 > simplified
...
if ReplicatedStorage.Interaction:FindFirstChild("Ban") then -- Disable connection to when we are changing its ancestry or tamper with it.
    task.spawn(function()
...
if LP:FindFirstChildOfClass("Backpack") then -- tries to detect if we added HopperBins to our backpack
    task.spawn(function()
...
hookfunction(Anticheat_Env.backpackClean, function(...) -- this for some reason deletes other players backpacks
    return
end)
...
OldNameCall = hookmetamethod(game, "__namecall", function(...)
    ...
    if not checkcaller() then
        if (Self.Name == 'AddLog' or Self.Name == 'Ban' or Self.Name == 'DamageHumanoid') then
            return
        end
