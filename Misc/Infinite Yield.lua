...
if IY_LOADED and not getgenv().IY_DEBUG then
	error("Infinite Yield is already running!",0)
	return
end

if not IY_LOADED and _senv then getgenv().IY_LOADED = true end
...
if ProtectInstance then -- if synv2
...
function getRoot(char)
	local rootPart = char:FindFirstChild('Humanoid').RootPart or char:FindFirstChild('Torso') or char:FindFirstChild('UpperTorso')
	return rootPart
end
